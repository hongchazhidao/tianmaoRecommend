...
//			
	 ...
	 static class Step3_Mapper extends Mapper<LongWritable, Text, Text, IntWritable>{
		...
		protected void map(LongWritable key, Text value, //第三步的输入是第二步的输出
				Context context)
			...
			for (int i = 0; i < items.length; i++) {
				...
				for (int j = 0; j < items.length; j++) { //只要用户对某2个商品产生点击及以上的行为,这2个商品就算同现一次 另外商品自己和自己也要算同现,因为用户可能没买过那个商品
