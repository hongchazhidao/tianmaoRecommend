...
u26	i276:1,i201:1,i348:1,i321:5,i136:1,
 * @author root
 *
 */
public class Step2 {
	 ...
	 static class Step2_Reducer extends Reducer<Text, Text, Text, Text>{
			...
			protected void reduce(Text key, Iterable<Text> i,
				...
				for(Text value :i){
					...
					action = ((Integer) (r.get(item)==null?  0:r.get(item))).intValue() + action; //一个用户对1个产品可能同时会产生如点击和购买的行为,那么就该用户对这个产品的喜爱度就要是这2个行为的喜爱度相加
					r.put(item,action);
				}
