	...
	static class FofMapper extends Mapper<Text, Text, Fof, IntWritable>{
		protected void map(Text key, Text value,
			...
			for (int i = 0; i < friends.length; i++) {
				...
				context.write(ofof, new IntWritable(0)); //用来在shuffle分组时去掉已经是好友的fof,如果组中有数据value是0,这个组就不用在reduce中输出了.
...
static class SortMapper extends Mapper<Text, Text, User, User>{ //MapOutputKey是User是因为要用friendsCount排序,MapOutputValue是User是因为reduce中要输出推荐的可能认识的好友的姓名和相关度
